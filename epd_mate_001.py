		...
		try:
			...
			with open(self.input_path, 'r', encoding='utf-8', errors='ignore') as fin, open(self.output_path, 'w', encoding='utf-8') as fout:
				for line in fin:
					...
					# Try to parse as FEN/EPD: some EPD lines may contain extra tokens, take first token(s)
					# Common EPD: <FEN> ; operations... So we try to parse the FEN from the line start up to the 6th space.
					# A FEN has 6 fields separated by spaces.
					...
					# Analyse with engine
					try:
						# Use a time-limited depth-limited analysis
						...
						# else: not a mate or mate too far
			...
			self.progress_callback(100, processed, total_positions)
			self.eta_callback(0)
